...
export default function AnimatedHeader() {
  ...
  return (
    <>
      <header
        ref={headerRef}
        className="fixed inset-x-0 top-0 z-50 border-b border-transparent px-6 py-6 md:px-12"
        style={{ backgroundColor: "rgba(11,13,16,0)" }}
      >
        <div className="mx-auto flex max-w-content items-center justify-between">
          <Link href="/" className="font-display text-lg tracking-tight text-paper">
            <span className="font-light">choisis</span>
            <span className="font-normal text-fog">ta</span>
          ...
          <div className="flex items-center gap-4 text-paper">
            <button aria-label="Rechercher" className="hidden transition-opacity hover:opacity-70 md:block">
              <Search size={20} strokeWidth={1.5} />
            </button>
            <Link href="/compte" aria-label="Compte" className="hidden transition-opacity hover:opacity-70 md:block">
            ...
            >
              ...
              {count > 0 && (
                <span
                  ref={badgeRef}
                  className="absolute -right-2 -top-2 flex h-4 w-4 items-center justify-center rounded-full bg-charge text-[10px] font-semibold text-paper"
                >
                  {count}
                </span>
              )}
            </button>
            <button aria-label="Ouvrir le menu" onClick={() => setMenuOpen(true)} className="md:hidden">
