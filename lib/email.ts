...
function emailLayout(bodyHtml: string) {
  return `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #121418;">
      ...
      ${bodyHtml}
      <p style="font-size: 13px; color: #6b7280; line-height: 1.6; margin-top: 28px;">
        Une question ? <a href="https://www.choisistaborne.fr/contact" style="color: #121418;">Contactez-nous</a> depuis le site.
      </p>
...
async function send(input: { to: string; subject: string; html: string }) {
  ...
  if (!client) {
    console.warn(`RESEND_API_KEY is not configured — email "${input.subject}" skipped`);
    return;
  }

  try {
    await client.emails.send({ from: FROM_ADDRESS, to: input.to, subject: input.subject, html: input.html });
  } catch (error) {
    console.error("EMAIL_SEND_ERROR", input.subject, error);
  }
}
...
export async function sendWelcomeEmail(input: { to: string; name: string }) {
  ...
  await send({ to: input.to, subject: "Bienvenue chez CHOISISTABORNE", html });
}

export async function sendQuoteRequestConfirmationEmail(input: {
  to: string;
  firstName: string;
  productName?: string;
}) {
  const html = emailLayout(`
    <h1 style="font-size: 24px; font-weight: 500; margin: 12px 0 20px;">Votre demande a bien été reçue</h1>
    <p style="font-size: 15px; line-height: 1.6;">
      Bonjour ${input.firstName},
    </p>
    <p style="font-size: 15px; line-height: 1.6;">
      Nous avons bien reçu votre demande${input.productName ? ` concernant <strong>${input.productName}</strong>` : ""}.
      Un conseiller CHOISISTABORNE va l'étudier et vous recontactera prochainement, par téléphone ou par e-mail,
      pour affiner votre projet d'installation.
    </p>
    <p style="font-size: 15px; line-height: 1.6;">
      En attendant, vous pouvez continuer à consulter nos bornes de recharge et guides d'installation.
    </p>
    <p style="margin: 28px 0;">
      <a href="https://www.choisistaborne.fr/produits" style="background: #121418; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 999px; font-size: 14px; font-weight: 600;">
  ...
  await send({ to: input.to, subject: "Votre demande de devis CHOISISTABORNE a bien été reçue", html });
}
